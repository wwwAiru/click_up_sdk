...
Для извлечения конкретных кастом филдов из `TaskDto` используется `TaskDto.customField(String field_id)` (метод делает неявный каст по объявленному типу переменной).
